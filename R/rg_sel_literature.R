...
rg_sel_literature <- function (obj, batch, celltype, rgene, k = 2){
  ...
  for (gene in 1:length(hgene)) {
    for (type in 1:length(info_celltype)) {
      ...
      for (bat in 1:length(info_batch)) {
        cur_gene <- dat[dat$batch == info_batch[bat] &
                          dat$type == info_celltype[type], gene]
        cur_gene <- log(cur_gene + 1)
        response <- append(response, cur_gene)
        fact <- append(fact, c(rep(info_batch[bat], length(cur_gene))))
        num <- c(num, length(unique(cur_gene)))
      }
      if (sum(unique(response)!=0) > 1 & sum(num<=1)<length(info_batch) & sum(num==0)<(length(info_batch)-1)) {
        fact <- as.factor(fact)
  ...
  for (num in sort(unique(median_p$count), decreasing = TRUE)) {
    rg <- append(rg, rownames(median_p)[median_p$count ==
                                          num])
    if (length(rg) >= k) {
      break
    }
